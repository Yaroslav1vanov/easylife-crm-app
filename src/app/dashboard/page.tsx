    ...
    return (
      <div className="dashboard-v3" style={{ fontFamily: "'Manrope', sans-serif" }}>
        {seesAll && team.length > 0 && (
          <ViewAsSwitch team={team} viewAs={viewAs} setViewAs={setViewAs} />
        )}
        ...
        <div data-tour="dash-mydeadlines" className="card" style={{ padding: 16, borderRadius: 16, marginBottom: 18 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {myDeadlines.items.slice(0, 8).map(({ s, c, due }) => {
              const diff = Math.round((new Date(`${due}T00:00:00`).getTime() - new Date(`${todayIsoD}T00:00:00`).getTime()) / 86400000);
              ...
              return (
                <button key={s.id} onClick={() => router.push(path)}
                  ...
                  <div style={{ minWidth: 0, flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--t3)" }}>M{s.month_number} · #{s.order_num} · {s.hook_text || s.hook || "сценарий"}</div>
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "right" }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: 7, fontSize: 10, fontWeight: 800, background: `${color}1f`, color }}>сдать {fmtDateShort(due)}</div>
                    <div style={{ fontSize: 9, color, marginTop: 2 }}>{txt}</div>
                  </div>
                </button>
              );
            })}
