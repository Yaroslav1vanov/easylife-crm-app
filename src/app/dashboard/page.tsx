        ...
        <div className="card" style={{ borderColor: violations.length > 0 ? "rgba(239,68,68,0.3)" : "var(--brd)" }}>
          ...
          {violations.slice(0, showAllViolations ? violations.length : 6).map((v, i) => (
            <div key={i} onClick={() => router.push(`/dashboard/clients/${v.cid}`)}
              className="flex items-center gap-1 py-1.5 px-2 rounded cursor-pointer text-xs" style={{ borderLeft: `3px solid ${v.severity === "high" ? "var(--rd)" : "var(--or)"}`, marginBottom: 2 }}>
              <span className="font-semibold" style={{ color: "var(--t1)" }}>{v.client}</span>
              <span style={{ color: "var(--t2)", flex: 1 }}>— {v.msg}</span>
              <span className="badge" style={{ background: v.severity === "high" ? "rgba(239,68,68,0.15)" : "rgba(249,115,22,0.15)", color: v.severity === "high" ? "var(--rd)" : "var(--or)", padding: "3px 8px", fontSize: 10, fontWeight: 700 }}>
                {v.severity === "high" ? `-${v.days}д` : `⚠ ${v.days}`}
              </span>
            </div>
          ))}
          {violations.length > 6 && (
            <button onClick={() => setShowAllViolations(!showAllViolations)}
