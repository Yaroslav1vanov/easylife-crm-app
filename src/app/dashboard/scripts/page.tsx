  ...
  function exportForClient(ids: number[]) {
    ...
    const cards = chosen.map((s, i) => `
      <article class="c">
        ...
        ${s.post_caption ? `<div class="b"><div class="l cap">🟠 Описание к ролику</div><p>${esc(s.post_caption)}</p></div>` : ""}
        ${s.ref_url ? `<a class="ref" href="${esc(s.ref_url)}" target="_blank" rel="noopener noreferrer">▶ Посмотреть пример ролика</a>` : ""}
      </article>`).join("");
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;background:#f4f5f8;color:#1a1a2e;line-height:1.6;padding:32px 16px 64px;-webkit-font-smoothing:antialiased}.w{max-width:760px;margin:0 auto}header{text-align:center;margin-bottom:36px}header h1{font-size:26px;margin-bottom:6px;letter-spacing:-.5px}header .s{color:#8a8aa0;font-size:14px}.c{background:#fff;border-radius:18px;padding:26px 28px;margin-bottom:22px;box-shadow:0 2px 18px rgba(40,30,80,.06);border:1px solid #ecebf3}.ch{display:flex;align-items:center;justify-content:space-between;margin-bottom:18px;padding-bottom:14px;border-bottom:1px solid #f0eff6}.num{font-weight:800;font-size:17px;color:#6a3df0}.cl{font-size:12px;color:#9a9ab0}.b{margin-bottom:18px}.b:last-child{margin-bottom:0}.l{font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px}.l.hook{color:#2f9be0}.l.body{color:#8a5cf0}.l.cta{color:#3aaf5c}.l.cap{color:#e08a2f}.ref{display:inline-flex;align-items:center;gap:7px;margin-top:16px;padding:10px 16px;border-radius:10px;background:#f3f0ff;border:1px solid #ddd4ff;color:#6a3df0;font-size:14px;font-weight:700;text-decoration:none}.ref:hover{background:#ebe5ff}.b p{font-size:16px;white-space:normal}footer{text-align:center;color:#b3b3c4;font-size:12px;margin-top:36px}@media print{body{background:#fff;padding:0}.c{box-shadow:none;border:1px solid #ddd;break-inside:avoid}}</style></head>
<body><div class="w"><header><h1>Сценарии — ${esc(title)}</h1><div class="s">${chosen.length} ${chosen.length === 1 ? "сценарий" : chosen.length < 5 ? "сценария" : "сценариев"} · ${today}</div></header>${cards}<footer>Подготовлено в EasyLife AI</footer></div></body></html>`;
  ...
  const scriptTour: TourStep[] = [
    ...
    { target: "scripts-board", title: "Доска — 4 колонки и как считается план", text: "Идея → Взято в работу → На согласовании → Согласовано. «Идея» — копилка идей и референсов, их может быть сколько угодно. Как только начинаешь писать конкретный сценарий клиенту — тащишь в «Взято в работу»: тут ему присваивается номер, и именно эти карточки считаются в плане месяца (не идеи). Сейчас откроем карточку и разберём, как писать.", action: closeModal },
    ...
    { target: "scripts-board", title: "③ Скинуть клиенту на утверждение", text: "Сценарии написаны и лежат в «Взято в работу». Жми «☑ Выбрать несколько» над доской → отметь галочками нужные → «📄 Для клиента» → скачается готовый HTML-файл. Его и отправляешь клиенту на утверждение.", action: closeModal },
    { target: "scripts-board", title: "④ Отправил → «На согласовании»", text: "Как только скинул файл клиенту — перетащи эти карточки в «На согласовании». Так тимлид и все видят: сценарии у клиента, ждём ответ. Если клиент просит правки — правишь прямо в полях карточки и снова отправляешь.", action: closeModal },
    { target: "scripts-board", title: "⑤ Клиент одобрил → «Согласовано»", text: "Когда клиент подтвердил сценарии — тащи их в «Согласовано». С этого момента они автоматически появляются у монтажёра в разделе «Монтаж» — дальше он делает по ним ролики.", action: closeModal },
  ];
