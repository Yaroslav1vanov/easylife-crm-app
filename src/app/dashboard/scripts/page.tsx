...
<body><div class="w"><header><h1>Сценарии — ${esc(title)}</h1><div class="s">${chosen.length} ${chosen.length === 1 ? "сценарий" : chosen.length < 5 ? "сценария" : "сценариев"} · ${today}</div></header>${cards}<footer>Подготовлено в EasyLife AI</footer></div></body></html>`;
  ...
  const scriptTour: TourStep[] = [
    ...
    { target: "scripts-board", title: "⑥ Отправил → «На согласовании»", text: "Как только скинул файл клиенту — перетащи эти карточки в «На согласовании». Так тимлид и все видят: сценарии у клиента, ждём ответ. Если клиент просит правки — правишь прямо в карточке (можно через «↻ Переделать») и снова отправляешь.", action: closeModal },
